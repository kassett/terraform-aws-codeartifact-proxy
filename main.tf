...
check "unique_domains" {
  assert {
    condition     = length(var.repositories) == length(toset([for hos in var.repositories : hos.hostname]))
    error_message = "There must be a unique hostname for each repository."
  }

  assert {
    condition     = length(concat([for hos in var.repositories : hos.additional_hosts])) == length(tolist(toset(concat([for hos in var.repositories : hos.additional_hosts]))))
